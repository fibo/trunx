...
import { route } from '../docs/routes.js'
  ...
  return [route.home]
    .concat(
      Object.keys(route).map((level1) =>
        Object.keys(route[level1]).map((level2) => route[level1][level2])
      )
    )
    .flat()
}
...
function sitemapContent({ baseUrl, route }) {
  return [
    ...
    routePaths({ route }).map((routePath) =>
      urlTagContent({ baseUrl, routePath })
    ),
    '</urlset>',
  ].join('\n')
}
...
async function generateSitemap() {
  const { homepage: baseUrl } = await readFile(
    packageJsonFilepath
